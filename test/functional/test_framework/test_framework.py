...
TMPDIR_PREFIX = "encocoin_func_test_"


class EncoCoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use encocoin-cli instead of RPC for all commands")
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in encocoin.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in encocoin.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "encocoind"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run encocoinds:
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one zerocoin for each denom (tot 6666 XNK) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 zerocoins, one for each denom (tot 6666 XNK), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 XNK), change = 6750 - 6666 - fee
    ...
    ### EncoCoin Specific TestFramework ###
    ###################################
    ...
    def test_PoS_chain_balances(self):
        ...
        # - Nodes 2 and 3 have 6666 XNK worth of zerocoins
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zXNK inputs when fDoubleSpend is false. It needs to
...
class ComparisonTestFramework(EncoCoinTestFramework):
...
'''
EncoCoinTestFramework extensions
'''

class EncoCoinTier2TestFramework(EncoCoinTestFramework):
