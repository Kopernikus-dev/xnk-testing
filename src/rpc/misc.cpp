...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total encocoin balance of the wallet\n" // (excluding zerocoins)\n"
...
/*    UniValue zxnkObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zxnkObj.pushKV(std::to_string(denom), ValueFromAmount(0));
        else
            zxnkObj.pushKV(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN)));
    }
    zxnkObj.pushKV("total", ValueFromAmount(GetZerocoinSupply()));
    obj.pushKV("zXNKsupply", zxnkObj); 
    * */
#ifdef ENABLE_WALLET
    if (pwalletMain) {
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for EncoCoin cold staking -only if is standard address-\n"
