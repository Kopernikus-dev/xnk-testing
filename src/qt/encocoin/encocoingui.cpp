...
const QString EncoCoinGUI::DEFAULT_WALLET = "~Default";

EncoCoinGUI::EncoCoinGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void EncoCoinGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &EncoCoinGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &EncoCoinGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &EncoCoinGUI::showHide);
    ...
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &EncoCoinGUI::execDialog);
...
void EncoCoinGUI::setClientModel(ClientModel* _clientModel)
{
...
#ifndef Q_OS_MAC
void EncoCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EncoCoinGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
bool EncoCoinGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("EncoCoin Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
bool EncoCoinGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    connect(addressesWidget, &AddressesWidget::message,this, &EncoCoinGUI::message);
...
static bool ThreadSafeMessageBox(EncoCoinGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void EncoCoinGUI::subscribeToCoreSignals()
{
...
void EncoCoinGUI::unsubscribeFromCoreSignals()
{
