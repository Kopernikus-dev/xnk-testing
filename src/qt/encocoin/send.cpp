    ...
    btnContacts(new QPushButton())
{
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onXNKSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onXNKSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent XNKs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (walletModel && availableBalance > 0) {
        ...
        if (!res.result) {
            inform(tr("Error generating address to shield XNKs"));
            return;
        }
        ...
        inform(tr("You don't have any transparent XNKs to shield."));
    }
}
...
void SendWidget::onXNKSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto xnkAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(xnkAdd) || isStakingAddr) {
