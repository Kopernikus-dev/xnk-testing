...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZxnk)
{
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter EncoCoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "EncoCoin (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "EncoCoin (regtest).lnk";
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=EncoCoin (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=EncoCoin (regtest)\n";
        else
