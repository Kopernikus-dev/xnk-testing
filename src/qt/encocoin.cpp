...
#include "qt/encocoin/splash.h"
#include "qt/encocoin/welcomecontentwidget.h"
...
public:
    ...
    /// Get window identifier of QMainWindow (EncoCoinGUI)
...
{
    window = new EncoCoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &EncoCoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &EncoCoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(EncoCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(EncoCoinGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &EncoCoinGUI::handlePaymentRequest);
        connect(window, &EncoCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. EncoCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("EncoCoin Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
