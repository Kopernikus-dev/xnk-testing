...
void OnRPCPreCommand(const CRPCCommand& cmd)
{
    ...
    if (strWarning != "" && !GetBoolArg("-disablesafemode", false) &&
        !cmd.okSafeMode)
        throw JSONRPCError(RPC_FORXNKDEN_BY_SAFE_MODE, std::string("Safe mode: ") + strWarning);
}
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in XNK) allowed by GUI and RPC for the stake split threshold (default: %s)"),
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:43013"));
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. EncoCoin Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. EncoCoin Core is probably already running."), strDataDir));
        ...
        do {
            ...
            try {
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zxnk if already reindexed
                    RecalculateXNKSupply(1, fReindexZerocoin);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_ENCOCOIN && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_ENCOCOIN);
            }
        ...
        zwalletMain = new CzXNKWallet(pwalletMain);
