...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    entry.pushKV("bcconfirmations", confirms);      // DEPRECATED in 3.1.99
...
UniValue viewshieldedtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                ...
                "      \"address\" : \"xnkxaddress\",     (string) The EncoCoin address involved in the transaction\n"
...
UniValue shieldedsendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "      \"amount\":amount    (numeric, required) The numeric amount in " + "XNK" + " is the value\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() > 4 ))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in XNK received for this wallet.\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in XNK received for this wallet in P2CS contracts.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shielded EncoCoin addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The encocoin address (either transparent or shielded) is the key,\n"
            "                                     the numeric amount in XNK is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue ListReceived(const UniValue& params, bool by_label)
{
    ...
    for (auto& itAddrBook = itAddr; itAddrBook.IsValid(); itAddrBook.Next()) {
        ...
        if (by_label) {
            ...
            obj.pushKV("bcconfirmations", nConf);       // DEPRECATED in 3.1.99
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"encocoinaddress\",    (string) The encocoin address of the transaction.\n"
            "    \"category\":\"category\",      (string) The transaction category (send|receive|orphan|immature|generate).\n"
            "    \"amount\": x.xxx,          (numeric) The amount in XNK. This is negative for the 'send' category, and positive\n"
