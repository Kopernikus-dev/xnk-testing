...
UniValue listmintedzerocoins(const JSONRPCRequest& request)
{
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.pushKV("zXNK ID", m.hashPubcoin.GetHex());     // PubCoin
...
UniValue mintzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Encocoin to convert to zXNK\n"
...
UniValue DoZxnkSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
