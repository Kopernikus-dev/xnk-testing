...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++) {
        ...
        else if (fMasterNode && tx->vout[i].nValue 50000 * COIN) { // else if (fMasterNode && vout[i].nValue == GetMNCollateral() * COIN) {
        ...
        const bool fIncludeLocked) const
{
    ...
    if (nCoinType == ONLY_10000 && output.nValue != 50000 * COIN) return res; // if (nCoinType == ONLY_10000 && output.nValue != GetMNCollateral() * COIN) {
